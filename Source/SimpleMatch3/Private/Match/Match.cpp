...
FMatchIdentity UMatch::Check(const std::vector<APiece*> MatchingPieces)
{
	...
	{
		const FMatchMatrix* Choreography = &MatrixChoreographies[ChoreographyIndex];

		// MatrixHelpers::DebugLogMatrix(Choreography);
		...
		for (int i = 0; i < MatchingPieces.size(); ++i)
		{
			...
			const FIntPoint StartingPieceIndex(MatchingPieces[i]->ParentI, MatchingPieces[i]->ParentJ);
			...
			for (int RowIndex = 0; RowIndex < Choreography->Rows.Num(); ++RowIndex)
			{
				...
				{
					//This should start checking from top left Element
